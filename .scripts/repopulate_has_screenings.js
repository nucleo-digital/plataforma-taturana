...
db.cities.update({}, {has_screening:false})
db.states.update({}, {has_screening:false})

db.films.find().sort({slug:1}).forEach(function(film) {
  film.screening && film.screening.forEach(function(screening) {
    var _slug = slug(screening.city + " " + screening.uf + " " + screening.s_country);
    res = db.cities.update(
      {slug: _slug},
