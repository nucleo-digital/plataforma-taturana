...
if (Meteor.isClient) {
  Template.screenings.helpers({
    ...
    states_options: function(films) {
      ...
      _.each(films, function(film) {
        _.each(film.screening, function(screening) {
          if (screening.uf && in_future(screening['date'], screening['time'])) {
    ...
    cities_options: function(films) {
      ...
      _.each(films, function(film) {
        _.each(film.screening, function(screening) {
          if (screening.city && in_future(screening['date'], screening['time'])) {
  ...
  function get_future_films() {
    ...
    _.each(films, function(film) {
      ...
      _.each(screenings, function(screening) {
        if (in_future(screening['date'], screening['time'])) {
   ...
   * @return Boolean
   */
  function in_future(date, time) {
    var today = new Date(),
        scheduledDate = get_date(date, time);
