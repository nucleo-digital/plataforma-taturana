...
function scheduleNotifications(user_id, film_id, screening) {
  ...
  var flow = mailFlow(screening.date);
  console.log("flow: " + flow);

  if (flow != 'third-flow') {
    email_template = 'new-session-' + flow + '.html';
    email_data = {
    ...
    console.log(confirm_event_date_data);
...
function send_movie_date(screening_date) {
  var days = 9;
  if (mailFlow(screening_date) == 'first-flow') {
    days = 1;
  } else if (mailFlow(screening_date) == 'second-flow') {
    days = 2;
  }
...
function mailFlow(screening_date) {
  ...
  console.log("days_for: " + days_for);
  console.log("days_for > -3: " + days_for > -3);
  console.log("days_for < -3:" + days_for < -3);
  console.log("days_for > -9:" + days_for > -9);
  console.log(days_for < -3 && days_for > -9);
