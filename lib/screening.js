if (Meteor.isClient) {
  ...
  function saveScreening (film_id, isDraft) {
    ...
    var before_session = {
      ...
      movie: film.title,
      when: moment(new Date()).add(1, "minutes").toDate()
    };

    Meteor.call('insertTask', before_session, function (error, result) {
      if (!error) {
        var thisId = result;
        Meteor.call('scheduleTask', thisId, before_session, beforeTemplate);
      }
    });
    ...
    var how_session = {
      ...
      data: moment(screening.date).format("L"),
      movie: film.title,
      when: moment(new Date()).add(70, "seconds").toDate()
    };

    Meteor.call('insertTask', how_session, function (error, result) {
      if (!error) {
        var thisId = result;
        Meteor.call('scheduleTask', thisId, how_session, howTemplate);
      }
    });
