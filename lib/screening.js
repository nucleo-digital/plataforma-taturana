if (Meteor.isClient) {
  ...
  function saveScreening (film_id, isDraft) {
    ...
    // Envia um email para o embaixador lembrando de enveiar o relatório
    ambassador = Meteor.users.findOne({_id: user_id});
    film = Films.findOne({_id: film_id});

    var dataText = "Olá " + ambassador.profile.name + "\rSua sessão do filme" +
        film.title + "foi agendada com sucesso! \rÉ muito importante para nós " +
        "que após sua sessão você envie o relatório.";
    ...
    Meteor.call('sendEmail', report_email);
    // Fim envio de email
