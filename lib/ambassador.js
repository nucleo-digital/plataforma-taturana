...
if (Meteor.isClient) {
  ...
  Template.ambassadorEdit.helpers({
    ...
    amount_film_screenings: function(){
      scrs = Films.screenings_by_user_id(Meteor.user()._id)
      past_scr = 0;
      people = 0;
      for (a = 0; a < scrs.length; a++) {
        if (scrs[a].date < today.getTime()){
          people = scrs[a].real_quorum + people
          past_scr++;
        }
      }
    ...
    amount_films: function(){
      return Films.by_user_id(Meteor.user()._id).length;
    }
  });
}
