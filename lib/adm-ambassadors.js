if (Meteor.isClient) {
  ...
  Template.admAmbassadors.events({
    "submit #ambassadors-filters": function (event) {
      ...
      if (hasValue(el.city)) { filters['profile.city'] = el.city.value; }
      if (hasValue(el.uf)) { filters['profile.uf'] = el.uf.value; }
