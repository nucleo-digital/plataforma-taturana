...
Films.inventory = function (film) {
  ...
  _.each(screenings, function (screening) {
    ...
    if (!('draft' in screening) || ('draft' in screening && screening.draft == false)) {
      ...
      inventory.sessions += (screening.date > today.getTime()) ? 1 : 0;chartist-plugin-legend.js
      // Espectadores por mês

      var real_quorum = ('real_quorum' in screening) ? screening['real_quorum'].replace(/[^0-9]/g, '') : 0;
      if (real_quorum > 0) {
